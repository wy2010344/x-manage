...
import { NOTION_FAV_VERSION, pushFavsUnpushed, restoreFavs } from './notionSync'
...
export function FavNotionPanel({ setFavs, showToast, storage }: Props) {
  ...
  const handleSave = async () => {
    ...
    setBusy(true)
    const probe = await listChildDatabases({ tokenOrUrl: url, notionVersion: NOTION_FAV_VERSION, rootPageId })
  ...
  const handlePush = async () => {
    setBusy(true)
    const r = await pushFavsUnpushed(storage)
    setLastPushedAt(readLastPushedAt(LAST_PUSHED_KEY))
    showToast(r.ok ? (r.message || `已推送 ${r.pushed} 条收藏`) : (r.error || '推送失败'))
    setBusy(false)
  }

  const handleRestore = async () => {
    setBusy(true)
    const r = await restoreFavs(storage)
    if (r.ok) {
      setFavs(await getAllFavs())
      showToast(`已从 Notion 恢复并合并 ${r.count} 条收藏`)
    } else {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 11, color: '#636e72', marginBottom: 8, lineHeight: 1.5 }}>
        全局配置（与「标签」共用一份）。填入代理 URL 与根页面链接后，系统会按作者自动创建「收藏 (@@handle)」子数据库，并每 30 分钟增量自动推送；遇到异常可用「从 Notion 恢复」拉取并合并回本地。
      </div>
